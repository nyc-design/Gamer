                    ...
                    let render = move |state: &mut State, now: Instant| {
                        ...
                        if let Err(_) = match state.create_frame() {
                            Ok((buf, render_result)) => {
                                ...
                                // Send frame callbacks BEFORE pushing to GStreamer.
                                // buffer_sender.send() is a sync_channel that blocks until
                                // GStreamer consumes the buffer. If we send callbacks after,
                                // Xwayland clients (which block on frame callbacks before
                                // rendering) starve → 1fps death spiral.
                                ...
                                // NOW push buffer to GStreamer (may block on sync_channel)
                    ...
                    let render = move |state: &mut State, now: Instant| {
                        ...
                        if let Err(_) = match state.create_secondary_frame() {
                            Ok((buf, render_result)) => {
                                ...
                                // Send frame callbacks BEFORE pushing to GStreamer (same fix as primary)
                        ...
                        if let Some(client) = client {
                            ...
                            match X11Wm::start_wm(
                                ...
                                Ok(wm) => {
                                    ...
                                    // Run xhost +local: to allow X11 clients to connect
                                    // without authorization. Required for Azahar running
                                    // in a separate Docker container with DISPLAY=:N.
                                        ...
                                        .output()
                                    {
                                        Ok(output) => {
                                            if output.status.success() {
                                                tracing::info!("xhost +local: enabled - X11 access control disabled");
                                            } else {
                                                tracing::warn!("xhost +local: failed");
                                            }
                                        }
                                        Err(e) => {
                                            tracing::warn!(?e, "Failed to run xhost");
                                        }
                                    }
                                }
                                Err(e) => {
